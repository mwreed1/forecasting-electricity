...
There appears to be an outlier so I looked at the end of the data and starting from year 2013

```{r clean-data}
data <- data %>% slice(1: nrow(data) - 1)
data <- filter(data,year(ds)>=2012)
```
...
full_join(data, forecast) %>%
  ...
  labs(
    x = "Date",
    y = "Electricity usage",
    title = "Electricity usage model for account MT_370"
  )
```
